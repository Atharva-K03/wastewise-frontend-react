...
const VehicleManagementDashboard = ({ onBack, onCreateVehicle, onUpdateVehicle, onDeleteVehicle }) => {
  ...
  const { vehicles, deleteVehicle } = usePickup();
  ...
  const routeTrucks = vehicles.filter(v => v.type === 'route truck').length;
  const pickupTrucks = vehicles.filter(v => v.type === 'pickup truck').length;
