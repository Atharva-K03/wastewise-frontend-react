...
const Dashboard = ({ onCreatePickup, onDeletePickup, onUpdatePickup }) => {
  ...
  const { pickups, getZoneName, getVehicleName, getWorkerName, deletePickup } = usePickup();
  ...
  const stats = [
    ...
    {
      title: 'Active Routes',
      value: new Set(pickups.map(p => p.zone)).size,
      icon: MapPin,
    ...
    {
      title: 'Today\'s Jobs',
      value: pickups.filter(p => p.frequency === 'Daily').length,
      icon: Clock,
                    ...
                    {pickups.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={9} className="text-center py-8 text-muted-foreground">
                          No pickups scheduled. Create your first pickup to get started!
                        </TableCell>
                      </TableRow>
                    ) : (
                      pickups.map((pickup) => (
